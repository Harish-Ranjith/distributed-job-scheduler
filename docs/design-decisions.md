...
Handlers have a configurable execution timeout (`WORKER_JOB_TIMEOUT_MS`, five minutes by default). Shutdown is bounded by `WORKER_SHUTDOWN_TIMEOUT_MS` (30 seconds by default); after the deadline the worker exits and active leases are left to expire and be recovered by the reaper. Handlers should be asynchronous and idempotent because JavaScript timers cannot run while a handler blocks the event loop.
